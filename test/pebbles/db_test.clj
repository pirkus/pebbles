...
(deftest find-progress-test
  ...
  (testing "Find progress with different client returns nil"
    (let [client-krn1 "krn:clnt:client-1"
          client-krn2 "krn:clnt:client-2"
          email "test@example.com"
          filename "test.csv"
          progress-data {:clientKrn client-krn1
                        :filename filename
                        :email email
                        :counts {:done 10 :warn 2 :failed 1}}]
...
(deftest find-all-progress-test
  (testing "Find all progress for a user in a client"
    (let [client-krn "krn:clnt:test-client"
          email "test@example.com"
...
(deftest find-all-progress-by-client-test
  (testing "Find all progress for a client"
    (let [client-krn1 "krn:clnt:client-1"
      ...
      (let [results (db/find-all-progress-by-client @test-db client-krn1)]
        (is (= 2 (count results)))
        (is (every? #(= client-krn1 (:clientKrn %)) results))
        (is (contains? (set (map :filename results)) "file1.csv"))
        (is (contains? (set (map :filename results)) "file2.csv"))
        (is (not (contains? (set (map :filename results)) "file3.csv")))))
    
    (testing "Find progress for client with no records"
      (let [results (db/find-all-progress-by-client @test-db "krn:clnt:empty-client")]
        (is (empty? results))))))