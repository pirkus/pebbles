...
(defn kafka-integration-fixture
  ...
  [f]
  (println "Starting Kafka for integration tests...")
  (let [kafka-env (kafka-utils/fresh-kafka)]
    (binding [*kafka-env* kafka-env]
      (try
        (f)
        (finally
          (println "Cleaning up Kafka...")
...
(deftest ^:integration kafka-integration-test
  (testing "Send and process Kafka message end-to-end"
    (let [test-setup (kafka-utils/create-test-setup *kafka-env* "test-progress-topic")
      ...
      (try
        ...
        (kafka-utils/send-test-message (:producer test-setup) (:topic test-setup) test-message)
        
        ;; Create a temporary consumer to receive the message
        (let [consumer-props (doto (Properties.)
                              (.put "bootstrap.servers" (:bootstrap-servers test-setup))
          ...
          (try
            (.subscribe consumer [(:topic test-setup)])
        ...
        (finally
          (kafka-utils/cleanup-test-setup test-setup))))))

(deftest ^:integration kafka-consumer-component-test
  (testing "Kafka consumer component lifecycle"
    (let [test-setup (kafka-utils/create-test-setup *kafka-env* "component-test-topic")
          ...
          consumer (kafka-consumer/make-kafka-consumer
                    :bootstrap-servers (:bootstrap-servers test-setup)
                    :group-id "test-component-group"
                    :topic (:topic test-setup)
      ...
      (try
        ...
        (let [started-system (component/start system)]
          ...
          ;; Send a test message
          (kafka-utils/send-test-message (:producer test-setup) (:topic test-setup)
...
(deftest ^:integration http-kafka-payload-compatibility-test
  (testing "Same payload works for both HTTP and Kafka"
    (let [test-setup (kafka-utils/create-test-setup *kafka-env* "compatibility-test-topic")
      ...
      (try
        ;; Process via Kafka
        (kafka-utils/send-test-message (:producer test-setup) (:topic test-setup) payload)
...
(deftest ^:integration kafka-error-handling-integration-test
  (testing "Error handling in Kafka integration"
    (let [test-setup (kafka-utils/create-test-setup *kafka-env* "error-test-topic")]
...
(deftest ^:integration kafka-message-ordering-test
  (testing "Message ordering and processing"
    (let [test-setup (kafka-utils/create-test-setup *kafka-env* "ordering-test-topic")
      ...
      (try
        ...
        (doseq [msg messages]
          (kafka-utils/send-test-message (:producer test-setup) (:topic test-setup) msg))
